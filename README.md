...
Для запуска приложения: Ввести в терминале команду docker-compose up --build; Перейти по ссылке http://localhost:8501 для просмотра дашборда. Для загрузки потребуется некоторое время.